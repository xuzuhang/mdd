<template>
	<view>
		...
		<view  class="btnsGroup">
			<view @click="buyFlowers" class="buy btn">立即购买</view>
	...
	export default{
		onLoad(){
			for(let i = 0;i<categories.length;i++){
				this.categories[categories[i].name] = categories[i]
			}
			uni.getStorage({
				key:'flowers',
				success:(e)=>{
		...
		methods:{
			buyFlowers(){
				...
				if(this.categories[this.info.title]){
		
					uni.navigateTo({
						url:'../index/supplyList?id='+this.categories[this.info.title].id+'&name='+this.info.title
					})
				}else{
					uni.showToast({
						icon:'none',
