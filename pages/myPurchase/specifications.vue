	...
	<view>
		<view class="header"  v-for="(item,index) in infos" :key='index'>
			<view v-if="item.attributeType ==  'group'" class="headerItem">
				<view class="itemLeft">
					{{item.name}}<span v-if='item.isRequired' style="color:#ec6b66;">*</span>
				</view>
				<view class="itemRight">
					...
					<view v-if="item.arrs[item.index].attributeType ==  'input'" style="margin-top: 30upx;" class="itemRight">
						<view class="bigInput">
							<input v-model="item.arrs[item.index].values" type="text">
			...
			<view v-if="item.attributeType ==  'input'" class="headerItem">
				...
				<view class="itemRight">
					<view class="bigInput">
						<input v-model="item.values" type="text">
						<view>{{item.unit?item.unit:''}}</view>
					</view>
				</view>
			</view>
		...
		<view class="headerItem">
			...
			<view class="itemRight">
				<view class="offer">
					<view :class="offer[0]?'active':''" @click="offerchange(0)" class="offer-item">必填价格</view>
					<view :class="offer[1]?'active':''" @click="offerchange(1)" class="offer-item">必填图片</view>
				</view>
			</view>
		</view>
		<view class="headerItem">
			...
			<view class="itemRight">
				<view class="method">
					<view @click="methodchange(1,2)" :class="method==1?'active':''" class="methodItem">上车价</view>
					<view @click="methodchange(2,1)" :class="method==2?'active':''" class="methodItem">到货价</view>
				</view>
			</view>
		</view>
...
<script>
	export default{
		...
		methods:{
			...
			offerchange(e){
					var val = !this.offer[e]
					this.$set(this.offer,e,val)
			},
			...
			sure(){
				...
				for(let i = 0;i<arrs.length;i++){
					...
					obj.texts+=arrs[i].name+":"+ arrs[i].text+";"
					obj.val.push(objs)
				}
				obj.val = JSON.stringify(obj.val)
				obj.requirePrice = this.offer[0]
				obj.requirePhotos = this.offer[1]
				obj.payTerm = this.method==0?'':this.method
