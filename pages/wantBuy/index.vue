...
<script>
	...
	export default{
		...
		methods: {
			...
			newdate(){
					const date = new Date();
					let value = date.getTime() + 1000*60*60*24*7
					let start = date.getTime() + 1000*60*60*24*1
					let end = date.getTime() + 1000*60*60*24*180
					let valueDate = new Date(value)
					let startDate = new Date(start)
					let endDate = new Date(end)
					this.valueDate =  valueDate .getFullYear()+'-'+this.addzero(valueDate.getMonth() + 1)+'-'+this.addzero(valueDate.getDate())
					...
					this.obj.expires = this.valueDate
			},
			addzero(num){
				num = num > 9 ? num : '0' + num;
				return num
				...
				uni.getLocation({
					...
					fail:()=>{
						this.$myhttp.get(this.websiteUrl+'/my','',(res)=>{
								...
								this.$myhttp.get(this.websiteUrl+'/geocoder/latlng?address='+this.obj.address,'',(res)=>{
									this.obj.longitude = res.data.location.lng
									this.obj.latitude = res.data.location.lat
								},this)
						})
			...
			sures(){
				...
				this.$myhttp.post(this.websiteUrl+'/purchases',this.obj,(res)=>{
