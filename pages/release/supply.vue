<template>
	<view>
		...
		<cpimg ref="cpimg" @result="cpimgOk" @err="cpimgErr" :number="count" :fixOrientation="true" :size="500" :maxWidth="1000" :ql="1" type="url" />
		<myCityPicker 
		...
		<view class="header">
			...
			<view class="headerItem">
				...
				<view class="headerItemRight item3">
					<input :disabled='throttle' @input="changeInput('price',$event)"  placeholder-style="color:#c7c7cc;" placeholder="请输入货品价格" type="number">
					<view>元</view>
				</view>
			</view>
			<view class="headerItem">
				...
				<view class="headerItemRight item3">
					<input :disabled='throttle' @input="changeInput('quantity',$event)"  placeholder-style="color:#c7c7cc;" placeholder="请输入货品数量" type="number">
					<view>{{info.unit}}</view>
				</view>
			</view>
			<view class="headerItem">
			...
			<view class="headerItem">
				...
				<view class="headerItemRight item6">
					<textarea :disabled='throttle'  @input="changeInput('profile',$event)"  placeholder-style="color:#898989;font-size:35upx;"
	...
	export default{
		onBackPress(){
			if(this.backPress){
				uni.showModal({
				    title: '您即将关闭发布供应',
				    content: '发布供应”是您生意的开始，只要一分钟，动动手指，就可以让全国的采购商都看到您的供应，这一切都是免费的',
					cancelText:'继续编辑',
					confirmText:"暂不发布",
				    success: (res)=> {
				        if (res.confirm) {
		...
		onLoad(e){
			...
			this.obj.categoryId = this.info.id
			this.getspecisure()
		...
		methods: {
				...
				uni.getLocation({
					...
					fail:()=>{
						this.$myhttp.get(this.websiteUrl+'/my','',(res)=>{
								...
								this.$myhttp.get(this.websiteUrl+'/geocoder/latlng?address='+encodeURIComponent(this.obj.address),'',(res)=>{
			...
			onareasure(e){
				...
				this.$myhttp.get(this.websiteUrl+'/geocoder/latlng?address='+adress,'',(res)=>{
					this.obj.longitude = res.data.location.lng
					this.obj.latitude = res.data.location.lat
				},this)
			},
			...
			sures(){
				...
				this.$myhttp.post(this.websiteUrl+'/supplies',this.obj,(res)=>{
					if(res.data.code == 0){
						uni.navigateTo({
							url:'success?mode=supply'
						})
					}else{
						uni.showToast({
